# Function: segment_user_drillnew
...
segment_user_drillnew <-
  function(reactivated01,past28,active01){
    ...
    for (i in (1:length(reactivated01))){
      if (i <= 28){
        avgusesper28days <- 28*past28[i]/i
        pseudo_segment <- 
          (1:(length(category_breaks)))[
            category_breaks==
              min(
                category_breaks[avgusesper28days<=category_breaks]
              )
            ]
        nextentry <- paste("New",category_names[pseudo_segment],sep="_")
